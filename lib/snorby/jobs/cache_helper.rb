  ...
  module Jobs
    module CacheHelper
      ...
      def delete_sensor(sensor_id)
        # TODO: Include more tables with sensor dependency
        sql = [
      ...
      def sql_min_max
        sql = %{
          ...
          where 
          timestamp >= '#{@stime.to_s(:db)}' and timestamp < '#{@etime.to_s(:db)}' 
      ...
      def sql_event_count
        sql = %{
          ...
          where sid = #{@sensor.sid.to_i} and timestamp >= '#{@stime.to_s(:db)}' 
          and timestamp < '#{@etime.to_s(:db)}'
        }
      ...
      def sql_source_ip
        sql = %{
          ...
          and event.sid = iphdr.sid where timestamp >= '#{@stime.to_s(:db)}' 
          and timestamp < '#{@etime.to_s(:db)}'
      ...
      def latest_five_distinct_signatures
        sql = %{
          select signature from (
            select signature, MAX(timestamp) as timestamp from event group by signature order by timestamp desc limit 10
          ) as signature;
        }
