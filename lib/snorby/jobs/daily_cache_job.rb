    ...
    class DailyCacheJob < Struct.new(:verbose)
      ...
      def perform
        ...
        if Setting.autodrop_rollback?
          ...
          logit "Dropping old rollbacks", false

          Sensor.old_compilations.destroy
        ...
        Delayed::Job.enqueue(Snorby::Jobs::DailyCacheJob.new(false),
          :priority => 10,
          :run_at => Time.now.tomorrow.to_datetime.beginning_of_day)
      ...
      def send_yearly_report
        ReportMailer.yearly_report.deliver  if @stop_date.day == @stop_date.end_of_year.day
      end
      ...
      def send_users_reports
        User.all(:admin=> false).each do |user|
          sensors_sid_list = []
          Sensor.root.virtual_sensors.select{|sensor| Ability.new(user).can?(:read, sensor)}.each{|x| sensors_sid_list << x.sid; x.real_sensors.each{|real_sensor| sensors_sid_list << real_sensor.sid}}
          
          ReportMailer.yearly_report(sensors_sid_list, user).deliver if user.yearly? and @stop_date.day == @stop_date.end_of_year.day
          ReportMailer.monthly_report(sensors_sid_list, user).deliver if user.monthly? and @stop_date.day == @stop_date.end_of_month.day 
          ReportMailer.weekly_report(sensors_sid_list, user).deliver if user.weekly? and @stop_date.day == @stop_date.end_of_week.day 
          ReportMailer.daily_report(sensors_sid_list, user).deliver if user.daily?  
        end 
      end
