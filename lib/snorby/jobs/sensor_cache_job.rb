  ...
  module Jobs
    ...
    class SensorCacheJob < Struct.new(:verbose)
      ...
      def perform
        ...
        Sensor.all(:domain => false).each do |sensor|
          @sensor = sensor
        ...
        begin
          ...
          Sensor.all(:domain => false).each do |x|
            last_cache = Cache.all(:sid => x.sid, :cid.not => nil, :order => [:cid.desc]).first
            x.update(:last_cid => last_cache.cid) unless last_cache.nil?
          end
        ...
        Delayed::Job.enqueue(Snorby::Jobs::SensorCacheJob.new(false),
          :priority => 5, :run_at => Time.now.to_datetime + 10.minutes)
      ...
      def split_events_and_process(start_time, end_time)
        ...
        if event.empty?
          ...
          auto_classification(start_time, end_time, @sensor.sid)
      ...
      def auto_classification(start_time, end_time, sensor_sid)

        AutoClassification.all(:order => [:position.desc]).each do |auto_class|

          options = {:timestamp.gte => start_time, :timestamp.lte => end_time}

          if auto_class.sid.nil?
            options.merge!({ :sid => sensor_sid })
          else
            options.merge!({ :sid => sensor_sid }) if Sensor.get(auto_class.sid).real_sensors.map(&:sid).include?(sensor_sid)
          end

          options.merge!({ :'ip.ip_src' => IPAddr.new(auto_class.ip_src) }) unless auto_class.ip_src.nil?
          options.merge!({ :'ip.ip_dst' => IPAddr.new(auto_class.ip_dst) }) unless auto_class.ip_dst.nil?

          if auto_class.sig_sid and auto_class.sig_gid
            signature_ids = Signature.all(:fields => [:sig_id], :sig_sid => auto_class.sig_sid, :sig_gid => auto_class.sig_gid).map{|s| s.sig_id}
            options.merge!({:sig_id => signature_ids})
          end

          Event.all(options).update(:classification_id => auto_class.classification_id)
          events_count = Event.all(:classification_id => auto_class.classification_id).count

          Classification.get(auto_class.classification_id).update(:events_count => events_count)
